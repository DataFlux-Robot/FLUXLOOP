...
| ⚡ **MICRO** | HPM6750 / RK3566 + 云 API | maker、学生、价格敏感开发者（推理走云） | ¥1,999 / $280 |
| 📦 **mini** | RK3588 / STM32MP257 边缘算力 | 已能通过传感器读到状态的硬件 | ¥5,999 / $845 |
...
| 🔧 **本地 AI 模块** *(选配)* | 板载 CUDA 算力——本地跑 LLM **和**多物理场仿真 | **数据不出端**的安全部署 | +¥5,999 / +$845 |
...
默认盒子负责控制与感知，**重 LLM 推理和 GPU 多物理场仿真在云端跑**。按量计费，刻意做薄：

- **LLM** ≈ DeepSeek 价格 **+ 3%**
- **多物理场仿真（GPU）** ≈ A100 租用价 **+ 5%**

我们只对 **harness、spec 和工具链**收一点点加价——不赚 token 的钱。想自托管？随你，开源的。需要全部本地以满足安全？**本地 AI 模块**让整个闭环跑在你自己硬件上，**数据零出端**。
...
**对一起开发、讨论想法、或投资感兴趣？** 欢迎联系——我们是两个技术 founder，每条都会看。
