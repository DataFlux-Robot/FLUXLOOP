...
每一档都跑同一个 **FLUXLOOP 大脑**（auto-research agent + 物理仿真）。变的是 **Agent 能感知和行动到什么程度**——阶梯从「读传感器」→「看硬件」→「移动着看」→「动手操作」→「设计它」逐级爬升。

| 档位 | 增加什么 | 解锁的问题 | 状态 |
|---|---|---|---|
| **mini** | FLUXLOOP 盒子 · 单模态 LLM | 已能通过可读传感器拿到工作状态的硬件（编码器、可直接读的遥测） | 🎯 众筹 |
| **pro** | + 固定外部摄像头 · 多模态 LLM | **没有可读状态传感器**的硬件（如作动器无位置反馈）——摄像头成为虚拟传感器，Agent *看见*状态 | 🎯 众筹 |
| **max** | + 可调角度机械臂摄像头 + 可调底座（类 [Kynooe](https://www.kynooe.com/)） | 单一固定视角不够时——**主动感知**：Agent 自己换位置，从多角度观测被遮挡的状态 | 🔮 进阶 |
| **ultra** | + 模块化人形机器人 | Agent 不只观测，还能**动手操作**硬件——而机器人本身也成为 auto-research 的研究对象 | 🔮 未来 |
| **Ulti** | + 自动化模块设计系统 | 系统**自己设计人形机器人的模块** → 可进化的通用人形 + 通用科研能力。2040 终局。 | 🔮 北极星 · 不发售 |

> **这条阶梯就是我们 roadmap 的实体化。** `mini` 和 `pro` 是我们最先拿去众筹的产品；`ultra` 和 `Ulti` 是我们在奔赴的愿景——明确标注，绝不在真东西之前预售。*（价格、目标金额、发货窗口、demo 随众筹一起公布——见 [一起来做](#-一起来做)。）*
...
- 🎁 **众筹（即将开始）** —— `mini` 和 `pro`（见[产品阵容](#-产品阵容)）最先上早期支持者计划。*（回报档位、价格、发货随上线公布——Watch 本仓库，第一时间通知。）*
